...
Edit and delete resolve their target from the user's own Truss diagram list instead of taking a title up front. Both start with `truss_list_diagrams`.

**Edit runs headless**, exactly like create: `truss_get_diagram` and `truss_apply_diagram_edit` authenticate with a cached agent token and call the Truss API directly — no browser tab, no window stealing focus. Delete still opens a tab, via `truss_delete_diagram_prompt`, because its final confirmation is an in-app dialog.

## Dispatch

Verb mapping lives in [SKILL.md](../SKILL.md#dispatch): create/draw/render → create; change/update/rename/add to/remove from → edit; delete/remove the whole diagram → delete. When create and edit are both plausible — "make me a diagram of the payments flow" while a *Payments Flow* diagram already exists — ask which one before running anything. Never guess between them.
...
`truss_list_diagrams` reads from a cache alongside that token, so resolving "which diagram did they mean" usually costs no round trip. The cache is refreshed whenever it is older than five minutes, and it is never the last word: an id that misses the cache triggers one fresh read inside `truss_get_diagram` before it reports that a diagram does not exist, so a diagram created since the last `truss_list_diagrams` call resolves normally.

## Announce the tab (delete only)

Before calling `truss_delete_diagram_prompt`, tell the user: "opening Truss to confirm the delete." Delete still drives its exchange through the browser and shows Truss's own confirm dialog.

## Step 1: call `truss_list_diagrams` and resolve the target

Resolve which diagram the user means:

- **Named it.** Match case-insensitively. An exact name match wins. Otherwise, if exactly one diagram's name contains the user's text as a substring, that one wins. Otherwise show the candidates and ask — never choose between two plausible matches on your own.
...
Once you have a `diagramId` from the list, use it in the calls below. Never assign or invent one of your own — `truss_get_diagram` and `truss_delete_diagram_prompt` both reject an id that was not in a listing.
...
Call `truss_apply_diagram_edit` with `{ diagramId, fingerprint, desiredGraph }`, where `fingerprint` is exactly the value `truss_get_diagram` returned — never invented — and `desiredGraph` is the complete graph after your change, not a diff. The tool applies the diff and animates it into the room; anyone with that diagram open in a browser watches it happen live, with the AI cursor, exactly as they would during a create. Nobody needs to be watching for the edit to land. On a stale fingerprint the tool retries once from a fresh read on its own before reporting failure.

## Step 2 (delete only): confirm by name

For delete there is nothing more to send once you call `truss_delete_diagram_prompt` — the browser shows its own final confirm dialog. Before calling it, confirm with the user in the terminal by quoting the **full diagram name** you resolved, never its position in the list ("Delete the diagram **Payments Flow**?", not "Delete #2?") — a mistyped digit must never destroy the wrong diagram. Only call the tool on an explicit yes.

`truss_delete_diagram_prompt` returns once the choice has been relayed into the browser tab — it does not wait for, or report, the human's actual click, and it cannot delete anything itself: only that dialog can, using the human's own session, not the agent token.

## Step 3: finish

Report the tool's error message to the user without inventing detail beyond it. Two are worth handling specifically:

- *"This diagram is being actively edited elsewhere"* — a collaborator changed the canvas mid-edit. Offer to retry; the tool already retried once from a fresh read.
- *"The agent chose a diagram we don't recognize"* — a bug in your own reasoning, not the user's problem. Call `truss_list_diagrams` again and use an id from its result.
