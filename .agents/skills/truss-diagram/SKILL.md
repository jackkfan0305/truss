---
name: truss-diagram
description: Create, edit, or delete a Truss system architecture diagram in the user's browser. Use when the user asks an agent to create, draw, visualize, render, change, update, rename, add to, remove from, or delete a system design in Truss. Creating requires a user-specified title and description; editing and deleting resolve the target from the user's own project list instead.
---
...
For **edit** or **delete**, stop here and follow [references/operations.md](references/operations.md) instead of the steps below.
...
7. Before launching, confirm the resolved origin serves *this* build: `curl -s -o /dev/null -w '%{http_code}' <origin>/agent/new` **and** `curl -s -o /dev/null -w '%{http_code}' <origin>/agent/pick` must both answer `200`. `proxy.ts` makes both `/agent/new` and `/agent/pick` public and bypasses the Clerk dev handshake, so only a build containing `app/agent/new/` and `app/agent/pick/` answers `200` on both. A `307` on either means that route is missing and Clerk gated the request first; the user still sees a 404, but only after the sign-in round trip. The launcher opens whatever is listening and reports success either way, so this check is the only thing standing between a wrong port and a 404.
