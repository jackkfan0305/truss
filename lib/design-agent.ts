...
import type { ActivityEmitter } from "@/lib/ai-activity-stream";
import {
  readCanvas,
  readChatHistory,
  type RoomReads,
 ...
 * the route validates the two agree before running the orchestrator.
 *
...
export interface DesignAgentPayload {
  ...
  roomId: string;
  /**
   * Which model to design with, chosen in the composer. Optional, and
   * re-validated here rather than trusted: an unknown one falls back to the
   * default instead of failing the design.
   */
  modelId?: string;
  /**
...
export interface DesignRunOptions {
   ...
   * them down saves two Liveblocks round-trips per turn. Absent, they are read
   * here.
   */
  reads?: RoomReads;
}
...
async function runDesignWithoutPresenceCleanup(
  payload: DesignAgentPayload,
  { runId, activity, reads }: DesignRunOptions
): Promise<DesignRunResult> {
  ...
  try {
    ...
    console.info("Design plan parsed", {
      roomId,
 ...
 * into one disclosure, so the sidebar renders a growing paragraph.
 */
async function generateDesign({
  ...
  if (thoughts === 0) {
    console.info("No thought summaries returned for this run", {
      modelId,
      thinkingLevel,
    });
  }
