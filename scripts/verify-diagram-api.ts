...
const jsonRequest = (body: string) =>
  new Request("http://localhost/api/diagrams", { method: "POST", body });
...
function checkNameParsing() {
  ...
  const created = (body: unknown) =>
    parseDiagramName(body, DEFAULT_DIAGRAM_NAME);

  assert.equal(created({}), DEFAULT_DIAGRAM_NAME, "absent name");
  assert.equal(created({ name: "  " }), DEFAULT_DIAGRAM_NAME, "blank name");
  assert.equal(created({ name: null }), DEFAULT_DIAGRAM_NAME, "null name");
  ...
  assert.equal(parseDiagramName({}, null), null, "rename without a name");
  assert.equal(parseDiagramName({ name: "" }, null), null, "rename to blank");
  assert.equal(parseDiagramName({ name: "Payments" }, null), "Payments");
}

function checkIdParsing() {
  assert.deepEqual(parseDiagramId({}), { ok: true, id: undefined }, "absent id");
  assert.deepEqual(parseDiagramId({ id: "" }), { ok: true, id: undefined });
  assert.deepEqual(parseDiagramId({ id: "checkout-service-a1b2c3" }), {
    ok: true,
  ...
  for (const id of [
    ...
    assert.deepEqual(parseDiagramId({ id }), { ok: false }, `rejects ${id}`);
  }

  assert.deepEqual(parseDiagramId({ id: 7 }), { ok: false }, "non-string id");
  assert.deepEqual(parseDiagramId(null), { ok: false }, "malformed body");
}
...
function checkRoomIdsPassValidation() {
  ...
  const names = [
    ...
    "Café Service",
    "V2 -- Auth!!",
    "x".repeat(120), // the longest name parseDiagramName accepts
...
function checkCollaboratorEmailParsing() {
  // Lowercased on the way in, because @@unique([diagramId, email]) is not.
  assert.equal(
...
/**
 * `GET /api/diagrams/:id/agent-graph` is owner-only and reads the live room,
 * not the autosaved snapshot — see `handleAgentGraphGet`. `authorizeDiagram`'s
...
async function checkAgentGraphReadSuccess(): Promise<void> {
  const response = await handleAgentGraphGet("diagram-1", agentGraphDependencies());
...
async function main() {
  ...
  console.log("✅ Diagram API request parsing verified");
}

main().catch((error) => {
  console.error("❌ Diagram API verification failed");
  console.error(error);
  process.exitCode = 1;
});
