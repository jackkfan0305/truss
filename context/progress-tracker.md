...
- Edge branching for diagram readability (spec: `docs/superpowers/specs/2026-08-23-diagram-edge-branching-design.md`). The prior layout stamped `sourceHandle` and `targetHandle` on edges, placing them at the four cardinal handle points. Edge labels still stacked in the same rank-gap column and parallel edges between one pair overlapped. The new routing:
  ...
  - Lanes are ordered by vertical travel descending — the furthest-travelling edge splits first — which is what makes a bundle's single-turn edges geometrically incapable of crossing each other. A parallel group (edges sharing both endpoints) needs a second, matching guarantee for its own bow: `parallelLaneY` orders each member's row by distance from the *source*, not by an arbitrary id sort, so the member whose lane splits closest to the source always takes the row farthest from it. Lane indices are computed at layout time and persisted on `data.lane` so node dragging never reshuffles a bundle.
