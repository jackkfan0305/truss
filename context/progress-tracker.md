...
- Edge branching for diagram readability (Tasks 1–9, spec: `docs/superpowers/specs/2026-08-23-diagram-edge-branching-design.md`). The prior layout stamped `sourceHandle` and `targetHandle` on edges, placing them at the four cardinal handle points. Edge labels still stacked in the same rank-gap column and parallel edges between one pair overlapped. The new routing:
  - Dedupes identical `source + target + label` triples before layout, keeping only the first.
  - Routes edges through an orthogonal trunk-and-comb model: each edge leaving a handle by the same direction shares a horizontal trunk for `TRUNK_MIN` units, then peels off at staggered split columns `LANE_STEP` (200) apart, each in its own lane.
  - Lanes are ordered by vertical travel descending — the furthest-travelling edge splits first — making bundle self-crossings geometrically impossible. Lane indices are computed from handle geometry at layout time and persisted on `data.lane` so node dragging never reshuffles a bundle.
  ...
  - Rank gaps are computed from the widest bundle: dagre runs twice to reserve exactly the width needed, clamped to the coordinate budget (±10,000 per the agent contract). A wider diagram is preferred to cramped label collisions.
  - `components/canvas/canvas-edge.tsx` reads lane indices and parallel-group size to route through `orthogonalPath` for parallel members or `getSmoothStepPath` with staggered `centerX` for others. `lib/graph-layout.ts` runs the two-pass dagre and assigns lanes. `lib/canvas-geometry.ts` adds `laneOrder`, `edgeSplitX`, `parallelLaneY`, `orthogonalPath`, and `edgeLabelPoint` helpers. `types/canvas.ts` gains `LABEL_GAP`, `TRUNK_MIN`, `LANE_STEP`, `PARALLEL_STEP`, `CORNER_RADIUS` and persists `lane` on edges.
