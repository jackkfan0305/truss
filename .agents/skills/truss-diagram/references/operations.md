...
Once you have a `diagramId` from the list, use it in the calls below. Never assign or invent one of your own — `truss_get_diagram` and `truss_delete_diagram` both reject an id that was not in a listing.
...
`truss_delete_diagram` deletes the diagram outright and returns once the server has finished. There is no second dialog behind it and no undo, so the confirmation in the terminal is the only one the user gets. Quote the **full diagram name** you resolved, never its position in the list ("Delete the diagram **Payments Flow**?", not "Delete #2?") — a mistyped digit must never destroy the wrong diagram. Only call the tool on an explicit yes.

It deletes only a diagram the linked user owns; anything else comes back as an error, not a silent no-op.
...
- *"We couldn't delete that diagram"* — the diagram is gone, or the linked user does not own it. Re-run `truss_list_diagrams` before saying anything about what happened to it.
